...
// ✅ FIXED LocationSearch (no internal query conflict)
const LocationSearch = ({ value, onChange, placeholder }) => {
  const [suggestions, setSuggestions] = useState([]);
  const [loading, setLoading] = useState(false);
  const [debouncedQuery, setDebouncedQuery] = useState(value || "");
  const [userSelected, setUserSelected] = useState(false); // 🚀 NEW FLAG

  // Debounce value updates
  useEffect(() => {
    const handler = setTimeout(() => setDebouncedQuery(value), 400);
    return () => clearTimeout(handler);
  }, [value]);

  // Fetch suggestions
  useEffect(() => {
    // ✅ Stop fetching after a manual selection
    if (!debouncedQuery || userSelected) {
      setSuggestions([]);
      return;
    }
    ...
    fetchLocations();
  }, [debouncedQuery, userSelected]);

  // ✅ When user types again → unlock selection
  const handleInputChange = (e) => {
    setUserSelected(false);
    onChange(e.target.value);
  };

  // ✅ When user selects from dropdown → lock fetch
...
export default function BookingForm({ selectedCar, onBookingSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => setShowSuccessPopup(false), 9000);
  ...
  return (
    <div className="w-full flex justify-center items-center py-4">
      <div className="w-[600px] p-8 m-8 rounded-xl shadow-2xl border border-yellow-400 bg-black/40 backdrop-blur-lg overflow-y-auto">
        {/* Tabs */}
        ...
        <AnimatePresence>
          {showSuccessPopup && (
            <motion.div
              ...
              className="w-full my-2 bg-gradient-to-r from-green-400 to-green-600 text-white px-5 py-4 rounded-xl shadow-2xl z-50 flex items-center gap-2 border-2 border-white/30"
            >
