...
// // ✅ Fixed LocationSearch Component
...
//   useEffect(() => {
//     // Stop fetching once user has selected a location
//     if (!debouncedQuery || isSelected) {
//       setSuggestions([]);
//       setLoading(false);
//       return;
//     }
...
//       if (response.ok) {
//         // ✅ Send booking details to your email using EmailJS
//         const templateParams = {
//           from_name: formData.name,
//           from_email: formData.email,
//           phone: formData.phone,
//           carType: formData.carType || "N/A",
...
// ✅ Location Search Component (OpenStreetMap API)
const LocationSearch = ({ value, onChange, placeholder }) => {
  const [query, setQuery] = useState(value || "");
  const [debouncedQuery, setDebouncedQuery] = useState(query);
  const [suggestions, setSuggestions] = useState([]);
  ...
  return (
    ...
    >
      ...
      >
        <motion.div
          whileHover={{ scale: 1.02 }}
          className="w-full max-w-[450px] p-4 rounded-xl shadow-lg border border-yellow-400 bg-black/40 backdrop-blur-lg"
        >
          <h1 className="text-white text-center text-[1.5rem] mb-4">
            Book Now
          </h1>
            ...
            >
              <div className="flex flex-col gap-3">
                ...
                <select
                  ...
                  onChange={handleChange}
                  required
                  className="p-2 rounded bg-black border border-gray-400"
                >
                  <option value="">Select Car Type</option>
                  {carTypes.map((type) => (
                    <option key={type} value={type}>
          ...
          <p
            className="text-sm text-blue-400 underline cursor-pointer mt-4 text-center"
            onClick={() => setShowTermsModal(true)}
          >
            Read Terms & Conditions
          </p>
      ...
      {showTermsModal && (
        <div className="fixed inset-0 flex justify-center items-center z-50 bg-black/70">
          ...
          >
            ...
            <p className="text-l mb-4">
              1. All toll, highway, and bridge fees are paid by the customer.
              <br />
              2. Car must be returned in the same condition.
              <br />
              3. Driver must follow traffic rules.
              <br />
              4. Cancellation should be done 24 hours before booking.
              <br />
              5. Customer is responsible for damages during rental.
              <br />
              6. Payments must be completed before pickup.
              <br />
              7. Any violation of terms may lead to fines.
            </p>
            <button
              onClick={() => setShowTermsModal(false)}
              className="absolute top-2 right-2 text-red-500 font-bold"
      ...
      <AnimatePresence>
        {showSuccessPopup && (
          <motion.div
            ...
            className="fixed top-10 right-10 bg-gradient-to-r from-green-400 to-green-600 
                       text-white px-5 py-4 rounded-2xl shadow-2xl z-50 
                       flex items-center gap-2 border-2 border-white/30"
          >
            <span className="w-5 h-5 bg-white rounded-full flex justify-center items-center text-green-600 font-bold animate-bounce">
